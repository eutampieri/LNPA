...
do
	...
	do
		...
		ovs-vsctl add-port SW$HOST eth-H$NET$HOST
...
ip netns add HEXT
ip link add veth0 address 00:00:00:31:31:31 type veth peer name eth-HEXT
ovs-vsctl add-port SWEXT eth-HEXT
ip l set eth-HEXT up
ip l set veth0 netns HEXT
ip netns exec HEXT ip l set veth0 up
ip netns exec HEXT ip a a 192.168.1.1/24 dev veth0

ip l a eth-EXT address 00:00:00:32:32:32 type veth peer name eth-GW-EXT
ovs-vsctl add-port SWEXT eth-GW-EXT
ip l set eth-GW-EXT up
ip l set eth-EXT netns GW
ip netns exec GW ip l set eth-EXT up
ip netns exec GW ip a a 192.168.1.2/24 dev eth-EXT
ip netns exec GW ip r a default via 192.168.1.1

# NAT
ip netns exec GW iptables -t nat -A POSTROUTING -j MASQUERADE
# Firewall
ip netns exec GW iptables -I FORWARD -s 10.0.1.0/24 -d 10.0.2.0/24 -j DROP
ip netns exec GW iptables -I FORWARD -s 10.0.2.0/24 -d 10.0.1.0/24 -j DROP

ip netns exec H11 ping -c 1 192.168.1.1
ip netns exec H11 ping -c 1 10.0.1.2
ip netns exec H11 ping -c 1 10.0.2.1
ip netns exec H11 ping -c 1 10.0.1.254
